...
@livewire('cart-component')
    ...
    @if(session()->has('success'))
        <div x-data="{ show: true }" x-show="show" x-transition class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">
            <span class="block sm:inline">{{ session('success') }}</span>
            <button type="button" @click="show = false" class="absolute top-0 right-0 p-3 text-green-700 hover:text-green-900">
                ...
                <div class="w-[18rem] p-0 bg-white rounded-lg shadow-md">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-lg font-medium">Rs. {{ $latest->productSizes[0]->metal_price + $latest->productSizes[0]->gemstone_price + $latest->productSizes[0]->making_charges + $latest->productSizes[0]->gst }}</span>
                            <span class="my-1 text-yellow-500">
                                @php $rating = $latest->review->first()->rating ?? 0; @endphp
                                @for ($i = 0; $i < $rating; $i++) <i class="fa-solid fa-star"></i> @endfor
                            </span>
                        </div>
                        <div class="flex items-center justify-between mt-2">
                            ...
                            <i 
                                x-data="{ isWishlisted: @js(in_array($latest->id, $wishlist)) }"
                                x-on:click="
            ...
            @foreach($highlyRated as $rated)
                <div class="w-[18rem] p-0 bg-white rounded-lg shadow-md">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-lg font-medium">Rs. {{ $rated->product->productSizes[0]->metal_price + $rated->product->productSizes[0]->gemstone_price + $rated->product->productSizes[0]->making_charges + $rated->product->productSizes[0]->gst }}</span>
                            <span class="my-1 text-yellow-500">
                        ...
                        <div class="flex items-center justify-between mt-2">
                            <a wire:navigate href="{{ url('shop-now/' . $rated->product->id) }}" class="bg-[#5C3422] text-white text-sm px-4 py-2 rounded hover:bg-[#4a2b1b]">Shop Now</a>

                            <i 
                                x-data="{ isWishlisted: @js(in_array($rated->product->id, $wishlist)) }"
                                x-on:click="
                                    isWishlisted = !isWishlisted;
