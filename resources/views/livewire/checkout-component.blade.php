<div class="container mx-auto my-2">
    ...
    <form wire:submit.prevent="placeOrder" class="space-y-6">
                ...
                <div class="space-y-2">
                    @foreach (auth()->user()->userAddresses as $addr)
                        <label class="flex items-center border border-gray-300 rounded-md p-2 cursor-pointer">
                            <input type="radio" wire:model.live="wAddressId" value="{{ $addr->id }}"
                                class="mr-2">
            ...
            @forelse($cartItems as $index => $cart)
                <div class="flex flex-col md:flex-row py-4 border-b last:border-0">
                    ...
                    <div class="md:w-1/4 px-2">
                        <img src="{{ asset('storage/' . $cart->product->images[0]) }}" alt="{{ $cart->product->name }}"
                    ...
                    <div class="md:w-3/4 px-3 space-y-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select wire:model.live="selectedSizes.{{ $index }}"
                                    ...
                                    @foreach ($cart->product->productSize as $size)
                                        <option value="{{ $size->id }}">{{ $size->size }}</option>
                                    @endforeach
                                </select>
                                @error("selectedSizes.{$index}")
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" wire:model.live="quantities.{{ $index }}" min="1"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                            ...
                            <div>
                                <label
                                    class="flex items-center {{ $cart->product->express_delivery_available ? '' : 'line-through' }}">
                                    <input type="checkbox" wire:model.live="express.{{ $index }}"
                                ...
                                @if ($cart->product->express_delivery_available)
                                    <p class="text-xs"><b>Charge:</b> Rs. {{ $cart->product->express_delivery_charge }}
                                    </p>
                                @endif
                            </div>
                            ...
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model.live="gift.{{ $index }}"
                                        class="mr-2 h-4 w-4 text-indigo-600 rounded">
        ...
        <button type="submit" class="bg-green-600 hover:bg-green-800 text-white px-6 py-2 rounded block mx-auto mt-4"
            {{ $cartItems ? '' : 'disabled' }}>
            Place Your Order
        </button>
    </form>
