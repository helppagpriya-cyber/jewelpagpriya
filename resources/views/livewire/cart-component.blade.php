                ...
                @if (Auth::user())
                    ...
                    @forelse(Auth::user()->carts as $cart)
                        <div class="bg-white shadow-md rounded-lg mb-3 p-3 relative"
                            ...
                            <a wire:navigate href="{{ url('product/' . $cart->id) }}"
                    ...
                    @if (Auth::user()->carts->count() > 0)
                        <div class="bg-white shadow-md rounded-lg p-4 mt-4">
                            ...
                            <p class="text-sm"><span class="font-medium">Delivery Charge:</span> Rs.
                                {{ $tdelivery }}</p>
                            <hr class="my-2 border-gray-200">
                            <p class="text-sm"><span class="font-medium">Total:</span> Rs. {{ $tprice + $tdelivery }}
                            </p>
                            <a wire:navigate href="/shop-all"
                                class="mt-3 w-full inline-flex justify-center items-center px-4 py-2 bg-brand text-white rounded hover:bg-brand-hover transition">Shop
                                Now</a>
                        </div>
                    @endif
                @endif
            ...
            <div class="flex-1 overflow-y-auto p-4">
                @if (Auth::user())
                    @forelse(Auth::user()->wishlists as $wishlist)
                        <div class="bg-white shadow-md rounded-lg mb-3 p-3 relative">
                            <a wire:navigate href="{{ url('product/' . $wishlist->id) }}"
                                class="flex items-center text-gray-900 no-underline">
                                <div class="w-1/3">
