                    ...
                    <div class="relative">
                        @foreach($product->images as $key => $img)
                            <div class="relative {{ $key == 0 ? 'block' : 'hidden' }}">
                                <img src="{{ asset('storage/' . $img) }}" class="block w-full h-[550px] object-cover" alt="Product image">
                            </div>
                        @endforeach
                    </div>
            ...
            <div>
                @if($productSizes)
                    @foreach($productSizes as $size)
                        <button type="button" class="border {{ $productSize->id == $size->id ? 'border-gray-900' : 'border-gray-300' }} px-3 py-1 rounded hover:bg-gray-100" wire:click="setSize({{ $size->id }})">{{ $size->size }}</button>
                    @endforeach
                @endif
                @if($product->productDiscounts->count() > 0)
                    @foreach($product->productDiscounts as $discount)
                        @if ($discount->start_date <= \Carbon\Carbon::now() && $discount->end_date >= \Carbon\Carbon::now())
                            <div class="flex my-4">
                                <p class="text-3xl font-bold">Rs. {{ $productSize->metal_price + $productSize->gemstone_price + $productSize->making_charges + $productSize->gst - $discount->discount }}</p>
                                <p class="line-through mx-3 text-gray-500">Rs. {{ $productSize->metal_price + $productSize->gemstone_price + $productSize->making_charges + $productSize->gst }}</p>
                            </div>
                        @endif
                    @endforeach
                @else
                ...
                <div class="flex flex-wrap -mx-2 mt-4">
                    ...
                    <a href="{{ url('shop-now/' . $product->id) }}" class="w-full md:w-5/12 mx-auto px-2 text-white bg-[#5C3422] py-2 rounded text-center hover:bg-[#4a2b1b]">Shop Now</a>
                </div>
                ...
                <table class="w-full border-collapse border border-gray-300 text-center">
                    ...
                    @if($product->metal_id)
                        ...
                        <tr>
                            ...
                            <td class="border border-gray-300 p-2">{{ $productSize->metal_purity }}</td>
                        </tr>
                        <tr>
                    ...
                    @endif
                    <tr>
                        <td class="font-bold border border-gray-300 p-2">Total GST</td>
                        <td class="border border-gray-300 p-2">{{ $productSize->gst }}</td>
                    </tr>
                </table>
            </div>
        </div>
