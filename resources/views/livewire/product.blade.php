    ...
    @if(session()->has('warning'))
        ...
        >
            ...
            <button 
                type="button" 
                @click="show = false"
                class="absolute top-0 right-0 p-3 text-yellow-700 hover:text-yellow-900"
                aria-label="Close"
            >
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                    ...
                    <div class="relative">
                        @foreach($product->images as $key => $img)
                            <div class="relative {{ $key == 0 ? 'block' : 'hidden' }}">
                                <img src="{{ asset('storage/' . $img) }}" class="rounded-lg block w-full h-1/2 object-cover " alt="Product image">
                            </div>
                        @endforeach
                    </div>
                    <button class="absolute top-1/2 left-0 transform -translate-y-1/2 p-4" type="button" data-bs-target="carouselExample" data-bs-slide="prev">
                        <span class="inline-block w-8 h-8 bg-black bg-opacity-50 rounded-full" aria-hidden="true"></span>
                        <span class="sr-only">Previous</span>
                    </button>
                    <button class="absolute top-1/2 right-0 transform -translate-y-1/2 p-4" type="button" data-bs-target="carouselExample" data-bs-slide="next">
                        <span class="inline-block w-8 h-8 bg-black bg-opacity-50 rounded-full" aria-hidden="true"></span>
            ...
            <div>
                ...
                <span>Select Product Size:</span>
                    @foreach($productSizes as $size)
                        ...
                        <button type="button" class="border {{ $productSize->id == $size->id ? 'border-red-900' : 'border-gray-300' }} px-3 py-1 rounded hover:bg-gray-100" wire:click="setSize({{ $size->id }})">{{ $size->size }}</button>
                    @endforeach
                @endif
                @if($product->productDiscounts->count() > 0)
                    @foreach($product->productDiscounts as $discount)
                         @if ($discount->start_date <= \Carbon\Carbon::now() && $discount->end_date >= \Carbon\Carbon::now()) 
                            <div class="flex my-4">
                                <p class="text-3xl font-bold">₹ {{ $productSize->metal_price + $productSize->gemstone_price + $productSize->making_charges + $productSize->gst - $discount->discount }}</p>
                                <p class="line-through mx-3 text-gray-500">₹ {{ $productSize->metal_price + $productSize->gemstone_price + $productSize->making_charges + $productSize->gst }}</p>
                            </div>
                        @endif 
                    @endforeach
                @else
                    <p class="text-3xl font-bold mt-4">₹ {{ $productSize->metal_price + $productSize->gemstone_price + $productSize->making_charges + $productSize->gst }}</p>
                @endif
                ...
                <div class="flex flex-wrap -mx-2 mt-4">
                    <div class="w-full md:w-1/2 px-2">
                        <button class="bg-yellow-500 text-white w-full py-2 rounded hover:bg-yellow-600" wire:click="AddToCart()">Add To Cart</button>
                    </div>
                    <a wire:navigate href="{{ url('shop-now/' . $product->id) }}" class="w-full md:w-5/12 mx-auto px-2 text-white bg-green-500 py-2 rounded text-center hover:bg-green-600">Shop Now</a>
                </div>
                ...
                <table class="w-full border-collapse border border-gray-300 text-center">
                    ...
                    <tr>
                        ...
                        <td class="border border-gray-300 p-2"> ₹{{ $productSize->gemstone_price+ $productSize->metal_price + $productSize->gst +$productSize->making_charges  }}/-</td>
                    </tr>
                </table>
            </div>
        </div>
    ...
    @forelse($product->review as $review)
        <div class="flex flex-wrap my-2">
            <div class="flex items-center">
                <img src="{{ asset('storage/' . ($review->user->avatar ?? 'img.png')) }}" class="rounded-full w-[50px] h-[50px] object-cover" alt="User Image">
            ...
            <div class="flex ml-1">
                @if($review->image)
                    @foreach($review->image as $img)
                        <img src="{{ asset('storage/' . $img) }}" class="rounded mx-1 my-0 p-0 w-[80px] h-[80px] object-cover" alt="User Image">
                    @endforeach
                @endif
            </div>
        </div>
